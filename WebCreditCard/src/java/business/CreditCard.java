...
public class CreditCard 
{
    ...
    {   
        ...
        while (acctno == 0) 
        {
            ...
            try 
            {       //for each new acct. no. generated see if that acct. no. file already exist
                BufferedReader in = new BufferedReader(new FileReader(path + "CC" + acctno + ".txt"));
                in.close();
                acctno = 0;
            } 
            catch (IOException e) 
            {  //if a file does not exist the new acct. not is good
                this.climit = 1000;
                cerrmsg = writeStatus();
    ...
    public CreditCard(int AccountNumber, String path) 
    {  //constructor for existing account number
        cerrmsg = "";
        cmsg = "";
        this.path = path;
        try 
        {
            ...
            BufferedReader in = new BufferedReader(new FileReader(path + "CC" + AccountNumber + ".txt"));
